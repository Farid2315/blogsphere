...
export function TravelDetail({ travelId }: TravelDetailProps) {
    ...
    const fetchTravel = async () => {
      try {
        ...
        if (data.success && data.data) {
          ...
          if (travelData.locationName) {
            try {
              const address = await convertCoordinateStringToAddress(travelData.locationName)
              setDisplayAddress(address)
            } catch (error) {
              console.error('Error converting address:', error)
              setDisplayAddress(travelData.locationName)
            }
          }
        } else {
          throw new Error(data.error || 'Failed to fetch travel details')
        }
      } catch (err) {
  ...
  if (!travel) {
    return (
      <div className="p-6 max-w-6xl mx-auto">
        <Card>
          <CardContent className="p-6 text-center">
            <p className="text-muted-foreground">Travel not found</p>
          </CardContent>
        </Card>
      </div>
    )
    ...
    <div className="p-6 max-w-6xl mx-auto">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* Travel Header */}
          <div>
            <h1 className="text-3xl font-bold text-foreground mb-2">{travel.title}</h1>
            <div className="flex items-center gap-4 text-muted-foreground">
              <span>{travel.address || displayAddress || travel.locationName}</span>
              {travel.rating && (
                <div className="flex items-center gap-1">
                  <Star className="h-4 w-4 fill-yellow-400 text-yellow-400" />
                  <span>{travel.rating.toFixed(1)}</span>
                </div>
              )}
              <span>{travel.likesCount} likes</span>
            </div>
          </div>

          {/* Travel Image */}
          <div className="relative">
            {travel.images?.[0] && travel.images[0].trim() && (travel.images[0].startsWith('/') || travel.images[0].startsWith('http')) ? (
              <Image
                src={travel.images[0]}
                alt={travel.title}
                width={800}
              ...
              <div className="w-full h-64 md:h-80 bg-gray-200 dark:bg-gray-800 rounded-lg flex items-center justify-center">
          ...
          <div>
            ...
            {travel.promotionLink && (
              <Button 
                variant="link" 
                className="text-secondary hover:text-primary p-0"
                onClick={() => window.open(travel.promotionLink, '_blank')}
              >
                Visit the promotion 🔗
              </Button>
            )}
          ...
          <div className="flex gap-4">
            {travel.callNumber && (
              <Button 
                ...
                onClick={() => window.open(`tel:${travel.callNumber}`, '_self')}
              >
                <Phone className="mr-2 h-5 w-5" />
                Call
              </Button>
            )}
            {(travel.companyWebsite || travel.promotionLink) && (
              <Button
                size="lg"
                variant="outline"
                className="border-secondary text-secondary-foreground hover:bg-secondary/20 bg-transparent px-8 py-3"
                onClick={() => window.open(travel.companyWebsite || travel.promotionLink, '_blank')}
              >
        ...
        <div className="space-y-6">
          {/* Action Buttons */}
          <div className="space-y-3">
            {travel.instagramHandle && (
              <Button 
                className="w-full bg-accent hover:bg-accent/80 text-accent-foreground"
                onClick={() => travel.instagramHandle && window.open(`https://instagram.com/${travel.instagramHandle.replace('@', '')}`, '_blank')}
              >
                {travel.instagramHandle}
              </Button>
            )}
          ...
          {/* Address */}
          {travel.branches && travel.branches.length > 0 && (
            <div>
              <h3 className="text-lg font-semibold text-foreground mb-3">
                {travel.branches.length === 1 ? 'Address' : 'Locations'}
              </h3>
              <div className="space-y-3">
                {travel.branches.map((branch, index) => (
          ...
          {travel.timings && (
            <div>
              ...
              <div className="space-y-2">
                {Object.entries(travel.timings).map(([day, time]) => (
                  time && (
          ...
          {travel.comments && travel.comments.length > 0 && (
            <div>
              ...
              <div className="space-y-3">
                {travel.comments.slice(0, 3).map((comment) => (
                  <Card key={comment.id} className="bg-card">
                    <CardContent className="p-3">
                      <div className="flex items-center gap-2 mb-2">
                        {comment.author.image && comment.author.image.trim() && (comment.author.image.startsWith('/') || comment.author.image.startsWith('http')) && (
                          <Image
