...
export function MusicGrid() {
  ...
  useEffect(() => {
    const fetchMusics = async () => {
      try {
        ...
        if (data.success) {
          const raw = (data?.data?.music ?? data?.data?.musics ?? []) as unknown
          const musicsData: Music[] = Array.isArray(raw) ? (raw as Music[]) : []
          setMusics(musicsData)
          
          // Convert coordinate strings to addresses
          const newAddressCache: { [key: string]: string } = {}
          for (const music of musicsData) {
          ...
          setError(data.error || 'Failed to fetch musics')
        }
      } catch (err) {
  ...
  if (musics.length === 0) {
    return (
      <div className="p-3 sm:p-4 lg:p-6">
        <div className="text-center py-12">
          <p className="text-muted-foreground mb-4">No musics found</p>
          <p className="text-sm text-muted-foreground">
            Be the first to add a music promotion!
          </p>
        </div>
      </div>
    )
      ...
      {!latitude && !longitude && (
        <div className="mb-6 p-4 bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-medium text-gray-800 dark:text-gray-100 mb-1">Find nearby musics</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">Allow location access to see musics sorted by distance</p>
            </div>
            <Button 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-6">
        {musics.map((music) => (
          <Link key={music.id} href={`/music/${music.id}`}>
            ...
            {/* Music Image slider with two visible images and arrows */}
            <div className="relative overflow-hidden" onDoubleClick={(e) => handleDoubleClick(music.id, e)}>
              {(() => {
                ...
                return (
                  <CardImageSliderMusic images={images}>
...
function CardImageSliderMusic({ images, children }: { images: string[]; children?: React.ReactNode }) {
