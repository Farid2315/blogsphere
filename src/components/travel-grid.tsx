...
export function TravelGrid() {
  ...
  const handleLike = (travelId: string, e?: React.MouseEvent) => {
    if (e) {
      e.preventDefault()
      e.stopPropagation()
    }
    setLikedPosts(prev => {
      ...
      if (newLiked.has(travelId)) {
        newLiked.delete(travelId)
      } else {
        newLiked.add(travelId)
      }
      return newLiked
    })
  }
  ...
  const handleDoubleClick = (travelId: string, e: React.MouseEvent) => {
    e.preventDefault()
    e.stopPropagation()
    handleLike(travelId)
  }
  ...
  const toggleOfferOverlay = (travelId: string, e: React.MouseEvent) => {
    e.preventDefault()
    e.stopPropagation()
    setShowOfferOverlay(prev => prev === travelId ? null : travelId)
  }
  ...
  useEffect(() => {
    const fetchTravels = async () => {
      try {
        ...
        if (data.success) {
          const raw = (data?.data?.travel ?? data?.data?.travels ?? []) as unknown
          const travelsData: Travel[] = Array.isArray(raw) ? (raw as Travel[]) : []
          ...
          for (const travel of travelsData) {
            if (travel.locationName) {
              try {
                const address = await convertCoordinateStringToAddress(travel.locationName)
                newAddressCache[travel.id] = address
              } catch (error) {
                console.error('Error converting address for travel:', travel.id, error)
                newAddressCache[travel.id] = travel.locationName
              }
            }
          }
          setAddressCache(newAddressCache)
        } else {
          setError(data.error || 'Failed to fetch travels')
        }
      } catch (err) {
  ...
  if (travels.length === 0) {
    return (
      <div className="p-3 sm:p-4 lg:p-6">
        <div className="text-center py-12">
          <p className="text-muted-foreground mb-4">No travels found</p>
          <p className="text-sm text-muted-foreground">
            Be the first to add a travel promotion!
          </p>
        </div>
      </div>
    )
      ...
      {!latitude && !longitude && (
        <div className="mb-6 p-4 bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-medium text-gray-800 dark:text-gray-100 mb-1">Find nearby travels</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">Allow location access to see travels sorted by distance</p>
            </div>
            <Button 
          ...
          <Link key={travel.id} href={`/travel/${travel.id}`}>
            ...
            <div className="flex items-center gap-2 p-1 pb-0 pl-5">
              <div className="w-7 h-7 rounded-full overflow-hidden bg-gray-200 dark:bg-gray-700 flex-shrink-0 ">
                {travel.author?.image && travel.author.image.trim() ? (
                  <Image
                    src={travel.author.image}
                    alt={travel.author?.name || "User"}
                    width={28}
                    height={28}
                  ...
                  <div className="w-full h-full bg-gradient-to-br from-blue-400 to-purple-500 flex items-center justify-center text-white text-xs font-semibold">
                    {(travel.author?.name || "U").charAt(0).toUpperCase()}
                  </div>
                )}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-xs font-semibold text-gray-900 dark:text-gray-100 truncate">
                  @{travel.author?.name || "TravelFindsbySarah"}
                </p>
              </div>
            ...
            {/* Travel Image slider with two visible images and arrows */}
            <div className="relative overflow-hidden" onDoubleClick={(e) => handleDoubleClick(travel.id, e)}>
              {(() => {
                const images = travel.images && travel.images.length > 0 ? travel.images : ["/placeholder.svg"]
                return (
                  <CardImageSliderTravel images={images}>
                    {(travel.offers.length > 0 || travel.promotionOfferTag) && (
                      <div className="absolute top-2 left-2">
                        <button
                          onClick={(e) => {
                            e.preventDefault()
                            e.stopPropagation()
                            toggleOfferOverlay(travel.id, e)
                          }}
                        ...
                        >
                          ...
                          {travel.promotionOfferTag || "GREAT OFFERS"}
                        </button>
                        {showOfferOverlay === travel.id && (
                          <div className="absolute top-full left-0 mt-2 bg-black/90 text-white p-3 rounded-lg shadow-xl z-10 min-w-[200px]">
                            {travel.offers.length > 0 ? (
                              <>
                                <p className="text-sm font-semibold mb-1">{travel.offers[0].title}</p>
                                <p className="text-xs text-gray-300">{travel.offers[0].description}</p>
                                {travel.offers[0].validTill && (
                                  <p className="text-xs text-orange-300 mt-1">Valid till: {travel.offers[0].validTill}</p>
                                )}
                              </>
                            ) : (
                              <p className="text-sm font-semibold">{travel.promotionOfferTag}</p>
                            )}
                          </div>
                        )}
                      </div>
                    )}
                  </CardImageSliderTravel>
                )
              })()}
            </div>
            ...
            <div className="p-2">
              ...
              <div className="flex items-center gap-1 mb-1">
                ...
                <span className="text-xs text-gray-600 dark:text-gray-400 truncate">
                  {addressCache[travel.id] || travel.locationName}
                </span>
              </div>
              ...
              {travel.offers.length > 0 && (
                <div className="mb-2">
                  <p className="text-xs text-orange-600 dark:text-orange-400 font-medium flex items-center gap-1">
                    <span>💡</span>
                    Special Offer: {travel.offers[0].title}
                  </p>
                </div>
              )}
                ...
                <div className="flex items-center gap-3">
                  <button
                    onClick={(e) => {
                      e.preventDefault()
                      e.stopPropagation()
                      handleLike(travel.id, e)
                    }}
                    className="flex items-center gap-1 hover:scale-110 transition-transform"
                  >
                    <Heart 
                      className={`h-4 w-4 ${likedPosts.has(travel.id) ? 'fill-red-500 text-red-500' : 'text-gray-600 dark:text-gray-400'}`}
                    />
                    <span className="text-xs text-gray-600 dark:text-gray-400">
                      {likedPosts.has(travel.id) ? travel.likesCount + 1 : travel.likesCount}
                    </span>
                  </button>
...
function CardImageSliderTravel({ images, children }: { images: string[]; children?: React.ReactNode }) {
  const [index, setIndex] = useState(0)
  
  // Filter out invalid URLs - be more strict with validation
  const validImages = images.filter(img => {
    if (!img || typeof img !== 'string') return false
    const trimmed = img.trim()
    if (trimmed.length === 0) return false
    // Check if it starts with / or http
    if (trimmed.startsWith('/') || trimmed.startsWith('http')) return true
    return false
  })
  const imagesToShow = validImages.length > 0 ? validImages : ["/placeholder.svg"]
  
  const img1 = imagesToShow[index % imagesToShow.length] || "/placeholder.svg"
  const img2 = imagesToShow[(index + 1) % imagesToShow.length] || "/placeholder.svg"

  const prev = (e: React.MouseEvent) => {
    e.preventDefault()
    e.stopPropagation()
    setIndex((i) => (i - 1 + imagesToShow.length) % imagesToShow.length)
  }
  const next = (e: React.MouseEvent) => {
    e.preventDefault()
    e.stopPropagation()
    setIndex((i) => (i + 1) % imagesToShow.length)
  }
