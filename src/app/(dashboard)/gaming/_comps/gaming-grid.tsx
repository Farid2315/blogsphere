...
export function GamingGrid() {
    ...
    const fetchGamings = async () => {
      try {
        ...
        if (data.success) {
          ...
          if (!Array.isArray(gamingsData)) {
            gamingsData = gamingsData ? [gamingsData] : []
          }

          setGamings(gamingsData as Gaming[])
          
          const newAddressCache: { [key: string]: string } = {}
          for (const gaming of gamingsData as Gaming[]) {
          ...
          setError(data.error || 'Failed to fetch gaming posts')
        }
      } catch (err) {
  ...
  if (gamings.length === 0) {
    return (
      <div className="p-3 sm:p-4 lg:p-6">
        <div className="text-center py-12">
          <p className="text-muted-foreground mb-4">No gaming posts found</p>
          <p className="text-sm text-muted-foreground">Be the first to add a gaming promotion!</p>
        </div>
      </div>
    )
  }
      ...
      {!latitude && !longitude && (
        <div className="mb-6 p-4 bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-medium text-gray-800 dark:text-gray-100 mb-1">Find nearby gaming</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">Allow location access to see gaming posts sorted by distance</p>
            </div>
            <Button 
