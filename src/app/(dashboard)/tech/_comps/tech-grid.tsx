...
export function TechGrid() {
      ...
      try {
        ...
        if (data.success) {
          ...
          if (!Array.isArray(techsData)) {
            techsData = techsData ? [techsData] : []
          }

          setTechs(techsData as Tech[])
          
          const newAddressCache: { [key: string]: string } = {}
          for (const tech of techsData as Tech[]) {
          ...
          setError(data.error || 'Failed to fetch tech posts')
        }
      } catch (err) {
        setError('Failed to fetch tech posts')
  ...
  if (techs.length === 0) {
    return (
      <div className="p-3 sm:p-4 lg:p-6">
        <div className="text-center py-12">
          <p className="text-muted-foreground mb-4">No tech posts found</p>
          <p className="text-sm text-muted-foreground">Be the first to add a tech promotion!</p>
        </div>
      </div>
    )
  }
      ...
      {!latitude && !longitude && (
        <div className="mb-6 p-4 bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-medium text-gray-800 dark:text-gray-100 mb-1">Find nearby tech</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">Allow location access to see tech posts sorted by distance</p>
            </div>
            <Button 
