...
export function TravelGrid() {
    ...
    const fetchTravels = async () => {
      try {
        ...
        if (latitude && longitude) {
          ...
          params.append('radius', '100000') // 100km radius
        }
        ...
        if (data.success) {
          ...
          if (!Array.isArray(travelsData)) {
            travelsData = travelsData ? [travelsData] : []
          }

          setTravels(travelsData as Travel[])
          
          // Convert coordinate strings to addresses
          const newAddressCache: { [key: string]: string } = {}
          for (const travel of travelsData as Travel[]) {
          ...
          setError(data.error || 'Failed to fetch travel posts')
        }
      } catch (err) {
        setError('Failed to fetch travel posts')
  ...
  if (travels.length === 0) {
    return (
      <div className="p-3 sm:p-4 lg:p-6">
        <div className="text-center py-12">
          <p className="text-muted-foreground mb-4">No travel posts found</p>
      ...
      {!latitude && !longitude && (
        <div className="mb-6 p-4 bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-medium text-gray-800 dark:text-gray-100 mb-1">Find nearby travel destinations</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">Allow location access to see travel posts sorted by distance</p>
            </div>
            <Button 
