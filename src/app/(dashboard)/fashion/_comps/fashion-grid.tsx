...
export function FashionGrid() {
    ...
    const fetchFashions = async () => {
      try {
        ...
        if (data.success) {
          ...
          if (!Array.isArray(fashionsData)) {
            fashionsData = fashionsData ? [fashionsData] : []
          }

          setFashions(fashionsData as Fashion[])
          
          const newAddressCache: { [key: string]: string } = {}
          for (const fashion of fashionsData as Fashion[]) {
          ...
          setError(data.error || 'Failed to fetch fashion posts')
        }
      } catch (err) {
        setError('Failed to fetch fashion posts')
  ...
  if (fashions.length === 0) {
    return (
      <div className="p-3 sm:p-4 lg:p-6">
        <div className="text-center py-12">
          <p className="text-muted-foreground mb-4">No fashion posts found</p>
          <p className="text-sm text-muted-foreground">Be the first to add a fashion promotion!</p>
        </div>
      </div>
    )
  }
        ...
        <div className="mb-6 p-4 bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400">Allow location access to see fashion posts sorted by distance</p>
            </div>
            <Button 
