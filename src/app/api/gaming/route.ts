...
export async function GET(request: NextRequest) {
  try {
    ...
    if (latitude && longitude) {
      ...
      // Filter gamings by approximate distance using bounding box
      ...
      // Use aggregation to get all gamings and calculate distance
      const nearbyGamings = await prisma.post.aggregateRaw({
        pipeline: [
          {
            $match: {
              domain: { $in: ["gaming", "style"] }
            }
          },
      ...
      // Get total count for all gamings
    ...
    // Fetch gamings with pagination
    const gamings = await prisma.post.findMany({
      where: {
        ...where,
    ...
    console.error('Error fetching gamings:', error)
    return NextResponse.json(
      { 
        success: false,
        error: 'Failed to fetch gamings' 
      },
      { status: 500 }
    )
  }
