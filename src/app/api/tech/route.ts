...
export async function GET(request: NextRequest) {
  try {
    ...
    if (latitude && longitude) {
      ...
      // Filter techs by approximate distance using bounding box
      ...
      // Use aggregation to get all techs and calculate distance
      const nearbyTechs = await prisma.post.aggregateRaw({
        pipeline: [
          {
            $match: {
              domain: { $in: ["tech", "style"] }
            }
          },
    ...
    // Fetch techs with pagination
    const techs = await prisma.post.findMany({
      where: {
        ...where,
